...
- 2026-09-18 (review fix): code-review found `controllerClearTargets()` defined but never called, contradicting the SKILL/docs clear behavior. Fixed: clear handler now imports `controllerClearTargets` and `rmSync`s each existing target (recursive+force, same error style as the log deletion) alongside the session log; added regression test "the clear handler actually deletes every controller clear target". Full suite: 303 pass / 0 fail / 1 skipped (pre-existing live-credential skip).
